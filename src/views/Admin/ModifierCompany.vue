        ...
        <v-expansion-panel v-for="company in companys" :key="company.id">
          <v-expansion-panel-title>
            <v-row align="center" class="spacer" no-gutters>
              ...
              <v-col class="text-no-wrap text-left" cols="5" sm="3">
                ...
                <strong>{{ company.specialite }}</strong>
              </v-col>
            </v-row>
          </v-expansion-panel-title>
          ...
          <v-expansion-panel-text>
            <v-card-text
              ...
              v-if="company.name != null"
              ><strong>Nom :</strong> {{ company.name }}</v-card-text
            >
            <v-card-text
              class="text-body-1 font-italic"
              v-if="company.last_name != null"
              ><strong>Prénom :</strong> {{ company.last_name }}</v-card-text
            >
            <v-card-text
              class="text-body-1 font-italic"
              v-if="company.civilite != null"
              ><strong>Civilité :</strong> {{ company.civilite }}</v-card-text
            >
            <v-card-text
              class="text-body-1 font-italic"
            ...
            <v-card-text
              ...
              ><strong>Phone :</strong> {{ company.phone }}</v-card-text
            >
            <v-card-text
              class="text-body-1 font-italic"
              v-if="company.email != null"
              ><strong>Email :</strong> {{ company.email }}</v-card-text
            >
            <v-card-text
              class="text-body-1 font-italic"
              v-if="company.specialite != null"
              ><strong>Spécilaité :</strong>
              {{ company.specialite }}</v-card-text
            >
            <v-spacer></v-spacer>
            <router-link
